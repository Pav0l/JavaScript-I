...
// Solution with .map:
// inventory.map(item => carModels.push(item.car_model));
// carModels.sort();
...
// Solution with .map:
// let carYears = [];
// inventory.map(item => carYears.push(item.car_year));
...
const getOldCars = year => {
    let oldCars = [];
    let carYears = getAllYears();
    for (i = 0; i < carYears.length; i++) {
        carYears[i] < year ? oldCars.push(carYears[i]) : i;
    }
    return oldCars;
}
console.log(getOldCars(2000));
// Solution with .filter, needs previous solution uncommented:
